    ...
    public function store()
    {
        ...
        foreach ($expenses as $expense) {
            $data = [
                'year' => $expense->year,
                'month' => $expense->month,
                'total' => Expense::where('client_id', $clientId)->where('month', $expense->month)->where('year', $expense->year)->sum('amount'),
        ...
        $income = DB::table('incomes')->where('month', $month)->where('year', $year)->where('client_id', $clientId)->sum('paid');
        $othersIncome = DB::table('others_incomes')->where('month', $month)->where('year', $year)->where('client_id', $clientId)->sum('amount');

        $openingBalance = DB::table('balances')->where('month', $month - 1)->where('year', $year)->where('client_id', $clientId)->first();
        $manualOpeningBalance = DB::table('balances')->where('month', $month)->where('year', $year)->where('client_id', $clientId)->first();

        $balance = $income + $othersIncome - $monthExp->total;

        if (!$openingBalance && !$manualOpeningBalance) {
            $totalIncome = $income + $othersIncome;
            $totalExpense = $monthExp->total;
        } else {
            $opening = $manualOpeningBalance ?? $openingBalance;
            $totalIncome = $opening->flag == 1 ? ($opening->amount + $income + $othersIncome) : $income + $othersIncome - $opening->amount;
            $totalExpense = $opening->flag == 0 ? $monthExp->total + $opening->amount : $monthExp->total;
            $balance += $opening->amount ?? 0;
        }
